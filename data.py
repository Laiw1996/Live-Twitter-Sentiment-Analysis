...
def collate_fn(batch):  # rewrite collate_fn to form a mini-batch
    ...
    sentiment_tensor = torch.LongTensor(batch_size).zero_()

    for i, index in enumerate(sorted_index):
        sentence_tensor[i][:lengths[i]] = torch.LongTensor(batch[index]['sentence'][:max_length])

    for i, index in enumerate(sorted_index):
        sentiment_tensor[i] = torch.autograd.Variable(torch.from_numpy(np.array(batch[index]['sentiment'])))

    packed_sequences = torch.nn.utils.rnn.pack_padded_sequence(Variable(sentence_tensor.t()),
                                                                   lengths)  # remember to transpose
    return {'sentence': packed_sequences, 'sentiment': sentiment_tensor}


def collate_fn_predict(batch):
    "when predicting sentence label in the url, data without its sentiment label"
    lengths = np.array([len(data['sentence']) for data in batch])
    sorted_index = np.argsort(-lengths)
    lengths = lengths[sorted_index]  # descend order
    ...
    sentence_tensor = torch.LongTensor(batch_size, int(max_length)).zero_()
    for i, index in enumerate(sorted_index):
        sentence_tensor[i][:lengths[i]] = torch.LongTensor(
            batch[index]['sentence'][:max_length])
    packed_sequences = torch.nn.utils.rnn.pack_padded_sequence(Variable(sentence_tensor.t()),
                                                                   lengths)  # remember to transpose
    return {'sentence': packed_sequences}
...
class sentimentDataset(Dataset):
    def __init__(self, vocab, dataset, train_size=0.7, test_size=0.3, train=True):  # dataset
        ...
        self.sentiment = self.dataset[:][0].tolist()
        self.sentence = self.dataset[:][1].tolist()
        self.vocab = vocab
...
class sentimentDatasetPredict(Dataset):
    def __init__(self, vocab, dataset):  # dataset
...
def build_vocab(vocab):
    word_vector = gensim.models.KeyedVectors.load_word2vec_format(
        fname='glove embedding/GoogleNews-vectors-negative300-SLIM.bin', binary=True)
